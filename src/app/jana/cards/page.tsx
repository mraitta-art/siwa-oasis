...
function CardDesignerModal({ editType, types, fields, sections, setEditType, saveTemplate, getTemplate }: any) {
  const type = types.find((t: any) => t.id === editType);
        ...
        <div className="grid-2">
          <div>
            ...
            <div className="grid-2" style={{ gap: '0.5rem' }}>
              {Object.values(CARD_LAYOUTS).map((l: any) => (
                <div 
                  key={l.id} 
              ...
              <div style={{ 
                ...
                <div className="section-groups">
                  {sections.map((sec: any) => {
                    const secFields = Array.from(new Map(fields.filter((f: any) => f.section_id === sec.id).map((f: any) => [f.name, f])).values()) as any[];
                    if (secFields.length === 0) return null;
                    
                    const allSelected = secFields.every((f: any) => editingTpl.visible_fields.includes(f.name));
                      ...
                      <div key={sec.id} style={{ marginBottom: '1.5rem', borderBottom: '1px solid #f3f4f6', paddingBottom: '1rem' }}>
                         <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                            ...
                            <button 
                              ...
                              onClick={() => {
                                const fieldNames = secFields.map((f: any) => f.name);
                                const next = allSelected 
                                  ? editingTpl.visible_fields.filter((f: any) => !fieldNames.includes(f))
                               ...
                               <input 
                                 ...
                                 onChange={() => {
                                   const next = editingTpl.visible_fields.includes(f.name) 
                                     ? editingTpl.visible_fields.filter((x: any) => x !== f.name)
                ...
                <div style={{ height: 250, background: '#eee', position: 'relative', display: 'flex', alignItems: 'flex-end' }}>
                   ...
                   <div style={{ padding: '1.5rem', position: 'relative', color: '#fff', width: '100%' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.25rem', marginTop: '0.5rem' }}>
                        {editingTpl.visible_fields.slice(0, 3).map((f: any) => (
                    ...
                    {editingTpl.layout === 'data_rich' ? (
                      <div style={{ marginTop: '1rem', borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                         ...
                         <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                            {editingTpl.visible_fields.map((f: any) => (
