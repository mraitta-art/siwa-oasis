...
export default function PasteImportPage() {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (data.preview?.basic?.name && !customSlug) {
        setCustomSlug(data.preview.basic.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''));
      }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <h1 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <span>📋 Smart Paste-to-Minisite Importer</span>
          </h1>
          <p style={{ margin: '0.35rem 0 0', color: '#64748b', fontSize: '0.9rem' }}>
            Paste raw text from Booking.com, TripAdvisor, or OTA listings. Our AI mapper automatically sorts it into structured section fields.
          </p>
        </div>
          ...
          {parsedData ? (
            <div style={{ background: '#fff', borderRadius: '20px', padding: '1.75rem', border: '1.5px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                <div>
                  <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#D4AF37', letterSpacing: '1px', textTransform: 'uppercase' }}>Structure Ready</div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '20px', padding: '4rem 2rem', border: '2px dashed #cbd5e1', textAlign: 'center', color: '#94a3b8' }}>
              ...
              <div style={{ fontSize: '0.85rem' }}>Paste any listing or hotel text on the left and click "Analyze & Organize Sections" to see the smart section breakdown.</div>
            </div>
          )}
        </div>
      </div>
