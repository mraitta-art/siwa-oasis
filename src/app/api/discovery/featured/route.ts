...
export async function GET(request: NextRequest) {
  try {
    // Fetch businesses with high subscription tiers (PLATINUM/GOLD)
    const businesses = await query(`
      SELECT id, name, type_id, custom_data, subscription_tier 
      FROM businesses 
      WHERE subscription_tier IN ('platinum', 'gold', 'premium')
      LIMIT 5
    `);
    ...
    businesses.forEach((biz: any) => {
      ...
      Object.keys(data || {}).forEach(sid => {
        ...
        if (sectionData.feature_on_main === true || sectionData.feature_on_main === 'true') {
          ...
          const photos = Array.isArray(sectionData.section_gallery) ? sectionData.section_gallery : [];
    ...
    if (slides.length === 0) {
      businesses.forEach((biz: any) => {
        ...
        const sid = Object.keys(data || {}).find(s => 
          (data[s].section_gallery && data[s].section_gallery.length > 0) || data[s].youtube_story
        );
        ...
        if (sid) {
          ...
          slides.push({
            ...
            mediaUrl: isYT ? section.youtube_story : (section.section_gallery[0]?.url || section.section_gallery[0]),
            title: biz.name.toUpperCase(),
