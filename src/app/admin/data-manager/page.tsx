...
export default function DataManagerPage() {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/data-manager/export');
      if (res.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const res = await fetch('/api/admin/data-manager/import', {
          method: 'POST',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
             ...
             <button className="btn btn-primary" onClick={async () => {
                ...
                window.location.href = `/api/admin/data-manager/csv?table=${table}`;
             }}>
        ...
        <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '8px', border: '1px dashed #cbd5e1', textAlign: 'center' }}>
           <input 
             ...
             onChange={async (e) => {
                ...
                setLoading(true);
                const res = await fetch('/api/admin/data-manager/csv', {
                  method: 'POST',
                  body: formData
                });
                setLoading(false);
