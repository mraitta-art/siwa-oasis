...
import { PageTemplate } from '@/lib/admin/page-builder-types';
...
export default function PageTemplatesPage() {
  ...
  async function loadTemplates() {
    try {
      const response = await fetch('/api/admin/page-builder/templates');
      if (response.ok) {
    ...
    if (
      ...
      try {
        const response = await fetch(
          `/api/admin/page-builder/templates/${templateId}`,
          {
            method: 'DELETE',
          }
        );
