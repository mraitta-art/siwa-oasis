...
function OrchestratorContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/admin/types').then(res => res.json()),
      fetch('/api/admin/vendors').then(res => res.json()),
      fetch('/api/admin/sections').then(res => res.json()),
      fetch('/api/admin/website/templates').then(res => res.json()).catch(() => [])
  ...
  useEffect(() => {
    if (state.selectedType) {
      fetch(`/api/admin/forms?type=${state.selectedType}`)
        .then(res => res.json())
        .then(fields => {
          ...
          {state.step === 'SUCCESS' && (
             <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ marginTop: '3rem', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                   <Link href="/admin/businesses" className="btn-premium">REGISTRY</Link>
