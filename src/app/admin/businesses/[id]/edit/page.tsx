...
export default function BusinessEditPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const tInfoRes = await fetch(`/api/admin/types?id=${business.type_id}`);
      const typeInfo = await tInfoRes.json();
      if (typeInfo.parent_id) {
        const pRes = await fetch(`/api/admin/types?id=${typeInfo.parent_id}`);
      ...
      const trRes = await fetch('/api/admin/tiers');
    ...
    <main style={{ background: '#f8fafc', minHeight: '100vh', color: '#1e293b', paddingBottom: '5rem' }}>
      ...
      <nav style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <Link href="/admin/businesses" style={{ width: 40, height: 40, borderRadius: '12px', background: '#fff', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', textDecoration: 'none' }}>
