...
export default function BusinessRegistryPage() {
  ...
  async function loadTypeSchema(typeId: string) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/forms?type=${typeId}`);
      if (res.ok) {
  ...
  return (
    <div className="animate-in">
      <div className="card-header">
        ...
        <Link href="/admin/forms" className="btn btn-primary">
              ...
              <tr key={b.id}>
                ...
                <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                       <Link href={`/admin/curation/${b.id}`} className="btn btn-xs btn-outline gold-border" title="Curate Content">
                         <i className="fas fa-magic"></i> CURATE
                       </Link>
                       <Link href={`/admin/businesses/${b.id}/edit`} className="btn btn-xs btn-outline" title="Edit Business DNA">
