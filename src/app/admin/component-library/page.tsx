...
export default function ComponentLibraryPage() {
  ...
  async function loadComponents() {
    ...
    try {
      const url = filterType === 'all'
        ...
        : `/api/admin/component-library?type=${filterType}`;
  ...
  async function toggleActive(id: number, currentStatus: boolean) {
    try {
      const res = await fetch(`/api/admin/component-library/${id}`, {
        method: 'PUT',
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
              ...
              >
                <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flexShrink: 0 }}>
                    <Link
                      href={`/admin/hero-carousel?componentId=${comp.id}`}
                      style={{
        ...
        <div style={{
          ...
          <Link
            href="/admin/blog/sidebar-builder"
            style={{
              background: 'linear-gradient(135deg, #3b82f6 0%, #60a5fa 100%)',
              padding: '2rem',
              borderRadius: '16px',
