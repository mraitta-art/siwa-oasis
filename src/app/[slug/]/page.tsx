...
/**
 * VANITY URL MINISITE PAGE
 * Renders the business profile based on the /slug structure.
 */
export default function VanityBusinessPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const bRes = await fetch(`/api/businesses/by-slug/${slug}`);
        ...
        console.error('Slug lookup failed:', e);
        setBiz(null);
      }
      setLoading(false);
    }
  ...
  if (loading) return (
    <div style={{ background: '#0f172a', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
      ...
      <span style={{ fontSize: '0.8rem', letterSpacing: '2px', fontWeight: 700, opacity: 0.5 }}>SYNCHRONIZING VANITY URL...</span>
    </div>
  );
