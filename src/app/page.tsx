      ...
      <section id="results" style={{ padding: 'clamp(4rem, 12vw, 12rem) clamp(1rem, 5vw, 4rem)', background: '#fff' }}>
        ...
        <div className="results-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 450px), 1fr))', gap: 'clamp(3rem, 8vw, 6rem)', maxWidth: 1500, margin: '0 auto' }}>
           {(searchResults.length > 0 ? searchResults : carouselSlides.filter(s => s.id !== 'siwa_intro').slice(0, 3)).map((item: any, i: number) => {
             ...
             const image = item.mediaUrl || (customData ? Object.values(customData).find((v:any) => v.section_gallery)?.[0]?.url : 'https://images.unsplash.com/photo-1544644181-1484b3fdfc62');
             const slug = item.slug || item.id;
             const isVerified = item.subscription_tier === 'gold' || item.status === 'active';

             return (
              <Link href={`/${slug}`} key={i} style={{ textDecoration: 'none' }} className="gallery-card-link">
                <div className="gallery-card" style={{ transition: 'all 0.6s cubic-bezier(0.23, 1, 0.32, 1)' }}>
                  <div className="card-media-container" style={{ height: 'clamp(350px, 40vw, 500px)', overflow: 'hidden', position: 'relative', borderRadius: '40px', boxShadow: '0 30px 60px -20px rgba(0,0,0,0.15)', background: '#f8fafc' }}>
                    ...
                    {/* ADMIN CHECK BADGE */}
                    <div className="premier-badge" style={{ 
                    ...
                    {/* DNA PILLS ON IMAGE */}
                    <div style={{ position: 'absolute', bottom: 30, left: 30, right: 30, display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                       {[era, material].map((tag, idx) => (
                         <span key={idx} style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', padding: '0.4rem 1rem', borderRadius: '30px', fontSize: '0.6rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>
                            {tag}
                         </span>
                       ))}
                    </div>
                  ...
                  <div style={{ padding: 'clamp(1.5rem, 4vw, 2.5rem) 0.5rem' }}>
                    ...
                    <h3 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.25rem)', fontWeight: 900, color: '#0a0f1d', marginBottom: '1rem', letterSpacing: '-1.5px', lineHeight: 1.1 }}>{name}</h3>
                    <p style={{ color: '#64748b', fontSize: '1.05rem', lineHeight: 1.7, maxWidth: '100%', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {summary}
                    </p>
                    <div style={{ marginTop: '2rem', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                       ...
                       <i className="fas fa-arrow-right" style={{ fontSize: '1.1rem', color: '#D4AF37', transition: 'transform 0.3s ease' }} className="arrow-icon"></i>
                    </div>
                  </div>
                </div>
              </Link>
