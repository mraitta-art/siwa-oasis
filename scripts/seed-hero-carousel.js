const { execute, query } = require('../src/lib/db');
...
async function seedAdminCarousel() {
  ...
  try {
    ...
    const vendors = await query(
      `SELECT id, name, slug, custom_data FROM businesses WHERE subscription_tier != 'free' OR curation_data->'$.is_featured' = true LIMIT 10`
    );
    ...
    vendors.forEach((biz) => {
      ...
      Object.entries(data).forEach(([secId, secData]) => {
